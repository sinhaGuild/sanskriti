...
export default function SubCategoryDetailPage({ params }: { params: Promise<{ subCategoryName: string }> }) {

    const { subCategoryName } = use(params)
    // console.log(categoryName)

    const { images, shouldColorize } = useSanskritiStore((state) => state)

    const filteredImages = images.filter(item => item.subCategory === subCategoryName.replace("%20", ' '));
    ...
    return (
        <AtomicLayout>
            <AtomicLayoutLeft>
                <AtomicHeading title={subCategoryName.replace("%20", ' ')} description={AtomicDescriptionSample} />
            </AtomicLayoutLeft>
            <AtomicLayoutRight>
